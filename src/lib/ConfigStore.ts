...
export default class ConfigStore {
    ...
    getConfigs() {
        ...
        return fs.readdirSync(`${cwd}`)
            .map(fileName => {
                const result = /^\.(?<config>.*)\.env$/i.exec(fileName) as Result | null;
                if (result) {
    ...
    find(configName: string) {
        return this.getConfigs().find(e => (e.groups && e.groups.config === configName));
    }
    ...
    setConfig(configName: string, newConfig: { [s: string]: string; } | ArrayLike<string>) {
