...
export async function formatFile(filepath: string) {
  ...
  if (path.parse(filepath).ext !== ".env") {
    throw new Error(
      `Require a dotenv file: '${path.relative(process.cwd(), filepath)}'`
    );
  }
...
export function formatingEnvConfig(bodyfile: string, filepath: string) {
  ...
  const configsLists = [
    ...
    ["HTTPS", httpsConfig],
    ...Object.entries(otherConfigs).sort(([a], [b]) =>
      a > b ? 1 : b > a ? -1 : 0
    ),
  ].filter(([, e]) => e.length);
  const enddata = configsLists.reduce((previousValue, [_configName, _opts]) => {
    ...
    previousValue += [`# ${configName}`, ...opts.map((a) => a.str), EOL].join(
      "\n"
    );
    return previousValue;
