import { Validator } from "../lib/arr-cursor";
...
export class VariableValue extends Base {
  ...
  prepare(bufferCursor: BufferCursor): void {
      ...
      while (bufferCursor.has()) {
        ...
        if (bufferCursor.current() === k.backslash) {
          ...
          switch (bufferCursor.current()) {
            ...
            // parse \r
            case 0x72:
              this.appendRaw(k.carriageReturn);
              break;
            // parse \t
            case 0x74:
              this.appendRaw(k.tab);
              break;
            // parse \v
            case 0x76:
              this.appendRaw(k.verticalTab);
              break;
            // parse \f
            case 0x66:
              this.appendRaw(k.formFeed);
              break;
            // parse \b
            case 0x62:
              this.appendRaw(k.backspace);
              break;
            // parse \\
            case 0x5c:
              this.appendRaw(k.backslash);
              break;
            // parse \"
            case 0x22:
              this.appendRaw(k.doubleQuotes);
              break;
            // parse \'
            case 0x27:
              this.appendRaw(k.singleQuote);
              break;
    ...
    const validatorPrevToNumberSign: Validator<typeof bufferCursor> = (
      cursor,
      actions
    ) => {
    ...
    while (bufferCursor.has()) {
      ...
      if (
        ...
        (([matched] = bufferCursor.clone().match(validatorPrevToNumberSign)),
        matched)
      ) {
        return;
      }
