...
import { ArgsType } from "../tdo/args-type";
import { EventsArrCursor } from "../tdo/events-arr-cursor";
...
export type Validator<T extends ArrCursor<any>> = (
  cursor: T,
  actions: typeof ArrCursorMatchActions
) => ArrCursorMatchActions | void;
...
export class ArrCursor<T, B extends T | undefined = T | undefined> {
  ...
  printDescriptor() {
    ...
    const txt = `position: ${this.position} current: ${current} json preview: ${
      isNumber(current) ? JSON.stringify(Buffer.from([current]).toString()) : ""
    }`;
    console.log(txt);
    return txt;
  }
  ...
  match(...validators: Validator<this>[]) {
    ...
    for (const [index, validator] of Object.entries(validators)) {
      const action =
        validator(this, ArrCursorMatchActions) ?? ArrCursorMatchActions.stop;
