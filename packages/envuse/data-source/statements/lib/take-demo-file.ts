...
const getState = () => {
  ...
  const demoPath = `${path.dirname(state.testPath)}/__demo__/${path.basename(
    state.testPath
  )}.yaml`;
...
export function takeDemoFile(name?: string) {
  ...
  if (!demo) {
    const err = new Error(
      `Cannot found demo ${JSON.stringify(
        testName
      )} on file ${relativeDemoPath}. Please create it on ${relativeDemoPath}`
    );
    Error.captureStackTrace(err, takeDemoFile);
    throw err;
  }
  ...
  if (typeof demo !== "string") {
    const err = new Error(
      `Demo ${JSON.stringify(
        testName
      )} on file ${relativeDemoPath} must be a string.`
    );
    Error.captureStackTrace(err, takeDemoFile);
    throw err;
  }
