...
class KeyGenerator {
  ...
  constructor(
    private keySymbolMemory: KeySymbolMemory
  ) { }
...
const filterCommentVariable = (element: Base): element is CommentOperator | Variable =>
...
export const diff = (blockBase: Block, blockCompare: Block) => {
  ...
  const elementsBase = blockBase.elementList
    .filter(filterCommentVariable);
  const elementsCompare = blockCompare.elementList
    .filter(filterCommentVariable);
  ...
  for (const [elementBase, elementCompare] of tableOfDiff.table.values()) {
    ...
    else if (elementBase instanceof Variable &&
